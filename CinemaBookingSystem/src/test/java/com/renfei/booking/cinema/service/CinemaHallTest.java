...
class CinemaHallTest {
    ...
    void bookSeatAfterCancellation() {
        ...
        // Book again the same seat
        Booking booking2 = new Booking("B5", 1, seats);
        cinemaHall.bookings.put(booking2.getBookingId(), booking2);
        assertTrue(cinemaHall.bookings.containsKey("B5"));
    }

    @Test
    void bookSeats_CustomStrategyOverflow() {
        // Book 9 seats starting at B3 (row 1, col 2)
        List<int[]> seats =
                customStrategy.selectSeats(
                        cinemaHall.seatingMap, 10, 10, 9, cinemaHall.getDefaultColPriority(), "B3");
        assertEquals(9, seats.size());
        // Should fill row 1 from col 2 to 9, then overflow to row 2 following default priority
        ...
        assertArrayEquals(new int[]{2, 4}, seats.get(8));
    }
    ...
    void reallocateSeats_SuccessfulReallocation() {
        ...
        assertArrayEquals(new int[]{2, 4}, selectedSeats.get(0)); // C5 is row 2, col 4
    }
    ...
    void testCustomSeatingStrategyFillRowAndOverflow() {
        ...
        // Should fill row 1 from col 2 to 4, then overflow to row
    ...
    void testCustomSeatingStrategyFillRowWithBookedSeats() {
        ...
        hall.bookCustom(booking, 1, "B4");

        booking = hall.bookDefault(3);
        booking = hall.bookCustom(booking, 3, "B3");
        String bookingId = booking.getBookingId();

        // Should skip B4 and fill B3, B5, then overflow to row C3
        assertEquals(3, booking.getSelectedSeats().size());
        assertArrayEquals(new int[]{1, 2}, booking.getSelectedSeats().get(0)); // B3
        assertArrayEquals(new int[]{1, 4}, booking.getSelectedSeats().get(1)); // B5
        assertArrayEquals(new int[]{2, 2}, booking.getSelectedSeats().get(2)); // C3
        ...
        for (int col : colPriority) {
            if (cinemaHall.seatingMap[0][col] == 1) {
                ...
                assertEquals(
                        ...
                        "Booked seats follow defaultColPriority strategy (row 0, preferred column)");
            }
        }
    }
    ...
    // =====================
    // Negative and Edge Cases TEST CASES
    // =====================
